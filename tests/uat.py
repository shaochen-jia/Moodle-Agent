...
try:
    ...
    # The same leak, one layer down and far worse. Playwright's sync driver is
    # bound to the thread that started it; leaving one running makes every
    # later call in that thread fail with "Sync API inside the asyncio loop".
    # Auto-sync is one long-lived process, so a single offline morning broke
    # every sync for the rest of the day.
    _stopped = []
    _sess_mod.sync_playwright = lambda: types.SimpleNamespace(
        start=lambda: types.SimpleNamespace(stop=lambda: _stopped.append(1)))

    class _LaunchFails(_sess_mod.MoodleSession):
        def _launch(self, headless, offscreen=False):
            raise RuntimeError("Could not launch any browser")

    try:
        with _LaunchFails(_scfg):
            pass
    except RuntimeError:
        pass
    check("N14 a session that fails to launch stops the browser driver",
          _stopped == [1],
